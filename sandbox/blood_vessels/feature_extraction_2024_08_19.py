...
box_mapper = statebuilder.BoundingBoxMapper(
    ...
    description_column="BranchTypeName",
)
box_layer = statebuilder.AnnotationLayerConfig(
    name="box", mapping_rules=box_mapper, data_resolution=[1, 1, 1]
)

rows = []
for id, box_info in box_params.iterrows():
    bounds_min_cg = (box_info[["x_min", "y_min", "z_min"]].values).astype(int)
    bounds_max_cg = (box_info[["x_max", "y_max", "z_max"]].values).astype(int)
    rows.append(
        {
            "point_a": bounds_min_cg,
            "point_b": bounds_max_cg,
            "BranchTypeName": box_info["BranchTypeName"],
        }
    )
...
sbs.append(
    statebuilder.StateBuilder(
        layers=[seg_layer],
        # client=client,  # view_kws={"zoom_3d": 0.001, "zoom_image": 0.0000001}
    )
)
